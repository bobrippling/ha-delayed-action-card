...
function deepQuerySelectorAll(selector, rootNode = document.body) {
    ...
    const traverse = (node) => {
        ...
        // @ts-ignore
        if (node.shadowRoot)
            [...node.shadowRoot.children].forEach(traverse);
    };
    traverse(rootNode);
    return nodes;
}
...
function findAndExtendCards(element, hass, tasks, entityId) {
    if (element.shadowRoot) {
        ...
        const config = homeAssistant.delayedActionConfig;
        if (config) {
        ...
        const childElements = element.shadowRoot.querySelectorAll(enabledCards.join(","));
        childElements.forEach((child) => findAndExtendCards(child, hass, tasks, entityId));
    }
}
...
function getEntityActions(entityId) {
    ...
    let result = [];
    const state = (_a = document.querySelector("home-assistant")) === null || _a === void 0 ? void 0 : _a.hass.states[entityId].state;
    let defaultIndex;
...
function openDialog(hass, entityId, alreadyScheduled) {
    ...
    dialogHeader.innerHTML = `
      ...
      <p style="margin: 9px 0 0 0" title='Delayed Action'>Delayed Action: ${(_a = hass.entities[entityId].name) !== null && _a !== void 0 ? _a : entityId}</p>
    `;
    ...
    content.innerHTML =
        ...
        <div class="custom-dialog-content">` + (alreadyScheduled ? "<label style='color: var(--accent-color);'>(Schedule already set)</label>" : "") + `
...
class DelayedActionCard extends HTMLElement {
    ...
    getFriendlyName(entityId) {
        ...
        return (_a = entity === null || entity === void 0 ? void 0 : entity.attributes.friendly_name) !== null && _a !== void 0 ? _a : entityId;
    }
...
console.info("%c  DELAYED-ACTION-CARD  \n%c  Version: 1.0.4       ", "background: #c0c0c0; color: black; font-weight: bold; padding: 5px 0;", "color: white; background: #333; font-weight: bold; padding: 5px 0;");
